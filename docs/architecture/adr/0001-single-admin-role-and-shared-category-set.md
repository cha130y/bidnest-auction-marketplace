...
## ประเด็นค้างที่ยังต้องตัดสินใจ

**ADM-005 ทับซ้อนกับ PROD-002** — ADM-005 ให้ admin ปิดการขายสินค้าที่ไม่เหมาะสมได้ (เปลี่ยนเป็น `INACTIVE`) แต่ PROD-002 ก็ให้ผู้ขายแก้สถานะสินค้าของตัวเองระหว่าง `ACTIVE`/`INACTIVE` ได้เช่นกัน ตามที่ SRS เขียนไว้ตอนนี้ **ผู้ขายจึงกด `ACTIVE` กลับได้ทันทีหลัง admin สั่งปิด** ซึ่งทำให้ ADM-005 ไม่มีผลบังคับจริง

SRS ยังไม่ได้ระบุทางออกไว้ ทีมต้องตกลงกันก่อน Dev 3 ลงมือทำ ADM-005 ทางเลือกที่ไม่ต้องแก้ schema คือ ตอนผู้ขายจะเปลี่ยนสถานะเป็น `ACTIVE` ให้ตรวจ `admin_actions` แถวล่าสุดของ product นั้น ถ้า action ล่าสุดคือ `DEACTIVATE_PRODUCT` ให้ปฏิเสธจนกว่าจะมี `REACTIVATE_PRODUCT` จาก admin (ทั้งสองค่ามีอยู่ใน `AdminActionType` แล้ว)

หมายเหตุ: query แบบนี้จะต้อง filter `admin_actions` ด้วย `productId` ซึ่งยังไม่มี index รองรับ ถ้าเลือกทางนี้ต้องเสนอเพิ่ม `@@index([productId, createdAt])` ให้ทีมอนุมัติก่อน เพราะเป็นการแก้ schema
