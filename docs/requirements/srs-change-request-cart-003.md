...
- **สถานะ:** 🟠 **ทีมเห็นชอบแล้ว — เหลือแก้เอกสาร** โค้ดอยู่ใน `dev` แล้ว แต่ SRS ยังเป็น v5 ที่เขียนว่าจ่ายทั้งตะกร้า
- **สร้างเมื่อ:** 2026-08-25
...
- **SRS ฉบับปัจจุบัน:** `BidNest-Auction and Marketplace-v5.pdf` (ยังไม่ได้แก้)
- **ข้อกำหนดที่กระทบ:** CART-002, CART-003, CART-004

> เอกสารนี้ทำตามรูปแบบเดียวกับ [srs-change-request-adm-005.md](./srs-change-request-adm-005.md)
> ข้อความในโค้ดบล็อกคือสิ่งที่ต้องนำไปใส่ใน SRS
>
> ⚠️ **ตอนนี้โค้ดกับ SRS ไม่ตรงกัน** — v5 ยังเขียนว่า *"จ่ายเงินครั้งเดียวรวมกันทั้งตะกร้า"*
> ใครอ่าน SRS อย่างเดียวจะเข้าใจผิด จนกว่าจะ export v6 ตามส่วนที่ 2
...
แต่ **SRS v5 เขียนไว้ชัดว่าจ่ายทั้งตะกร้า** — ประโยคใน CART-003 คือ
...
- [ ] แก้ 3 จุดในส่วนที่ 2 ที่ไฟล์ `.docx` ต้นฉบับ
- [ ] export PDF ด้วย `Save As → PDF` **ไม่ใช่** `Print to PDF` (ดูเหตุผลใน change request ของ ADM-005)
- [ ] วาง PDF ใหม่แทน `BidNest-Auction and Marketplace-v5.pdf` เป็น v6 แล้วอัปเดต path ของ SRS ใน `CLAUDE.md` ให้ชี้ไฟล์ใหม่
- [ ] อัปเดต story CART-002/003/004 ใน Jira

4 ข้อที่เหลือเป็นงานเอกสารทั้งหมด ไม่มีอะไรต้องแก้ในโค้ดอีก — ต้องทำใน Word โดยคนที่ถือไฟล์ `.docx` ต้นฉบับ